...
- **Keep updating the app when GitHub is hard to reach**: the in-app updater queries `https://libra.irises.cc/latest.json` (a Cloudflare R2 mirror) first, with GitHub as the fallback; minisign signature verification is unchanged, and the mirror itself is never trusted.
...
> | ------------------ | ------------------------------------------------------------------------------ |
> | Website            | **[libra.irises.cc](https://libra.irises.cc)**                                 |
...
### In-App Updates Now Go Through the libra.irises.cc Mirror

The updater now queries `https://libra.irises.cc/latest.json` first — a Cloudflare R2 mirror of the release manifest — with GitHub Releases as the fallback, so checking for and downloading updates no longer depends on GitHub being reachable. The mirror manifest points every platform's download at the same bucket, while the minisign signatures stay exactly as they were: a signature covers file content, not the URL, and every downloaded artifact is still verified against the built-in public key, so **the mirror itself is never trusted**. Publishing is handled by a release-gated sync workflow that only rewrites the root manifest when its tag really is GitHub's `releases/latest`, so the mirror can never push users back to an older version.
...
The updater's endpoint list is baked into the app binary, so existing installations still query only GitHub until they upgrade to a version containing this change; from then on the `libra.irises.cc` mirror comes first, with GitHub as fallback.
...
Visit [Releases](https://github.com/farion1231/cc-switch/releases/latest) and download the build for your system, or get it from the official site [libra.irises.cc](https://libra.irises.cc) (from this release on, downloads are distributed through Cloudflare edge nodes and no longer depend on GitHub being reachable).
