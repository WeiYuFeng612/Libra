...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.libra/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.libra/skill-backups/）
    ...
    fn resolve_uninstall_backup_source(skill: &InstalledSkill) -> Result<Option<PathBuf>> {
        // 返回值会被整目录复制进 ~/.libra/skill-backups/ 并由 get_skill_backups
...
mod tests {
    ...
    fn restore_from_backup_rejects_traversal_directory_in_metadata() {
        ...
        // SSOT 位于 {home}/.libra/skills，"../../pwned-restore" 若生效会写到 {home}/pwned-restore。
    ...
    fn uninstall_rejects_traversal_directory_from_db_row() {
        ...
        // 与 import_sql_string_for_sync 的效果一致）。SSOT = {home}/.libra/skills，
