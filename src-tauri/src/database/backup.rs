...
const LIBRA_SQL_EXPORT_HEADER: &str = "-- Libra SQLite 导出";
const LEGACY_SUMMER_SQL_EXPORT_HEADER: &str = "-- Summer SQLite 导出";
...
impl Database {
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        if trimmed.starts_with(LIBRA_SQL_EXPORT_HEADER)
            || trimmed.starts_with(LEGACY_SUMMER_SQL_EXPORT_HEADER)
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 Libra 导出的 SQL 备份文件。",
            "Only SQL backups exported by Libra are supported.",
        ))
    }
...
mod tests {
    ...
    #[test]
    fn import_still_accepts_legacy_export_headers() {
        for header in [
            super::LEGACY_SUMMER_SQL_EXPORT_HEADER,
            super::LEGACY_CC_SWITCH_SQL_EXPORT_HEADER,
        ] {
            assert!(Database::validate_cc_switch_sql_export(header).is_ok());
        }
    }
