...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.libra/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 旧数据只在首次启动时复制到 Libra 的独立目录；失败时不创建空数据库，
            // 保留旧目录供用户排查和重试。
            let legacy_data_migrated = crate::config::migrate_legacy_app_data_if_needed()?;
            panic_hook::init_app_config_dir(crate::config::get_app_config_dir());

            // 初始化日志（输出到 <app_config_dir>/logs/libra.log）
            {
                ...
                log::info!("=== Libra v{} started ===", env!("CARGO_PKG_VERSION"));
                if legacy_data_migrated {
                    log::info!("Legacy CCSwitch data migration completed successfully.");
                }
            }
            ...
            //
    // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 Libra 的工作方式。
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            codex_plus::get_libra_enhancements,
            codex_plus::set_libra_enhancement,
            codex_plus::apply_libra_enhancements,
            codex_plus::force_install_codex_plugin,
            codex_plus::set_computer_use_guard,
            codex_plus::libra_injection_asset_path,
...
Consider rolling back to an older Libra version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including libra.db)\n\
2) If you see “database version is newer”, please upgrade Libra\n\
