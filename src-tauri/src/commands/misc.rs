...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url("https://libra.irises.cc", None::<String>)
...
pub async fn is_portable_mode() -> Result<bool, String> {
    Ok(crate::config::get_portable_app_config_dir().is_some())
}

/// 关闭 ChatGPT 的全部进程，并从已安装位置重新启动。
#[tauri::command]
pub async fn restart_chatgpt() -> Result<String, String> {
    #[cfg(target_os = "windows")]
    {
        return tokio::task::spawn_blocking(restart_chatgpt_windows)
            .await
            .map_err(|e| format!("重启 ChatGPT 任务执行失败: {e}"))?;
    }

    #[cfg(not(target_os = "windows"))]
    Err("当前仅支持在 Windows 上重启 ChatGPT".to_string())
}

#[cfg(target_os = "windows")]
fn restart_chatgpt_windows() -> Result<String, String> {
    use std::process::Command;

    // 先确认可执行路径或 AppX 激活入口，再结束进程，避免定位失败时只关闭不重启。
    const SCRIPT: &str = r#"
...
if (-not $executablePath) {
  ...
  if (${env:ProgramFiles(x86)}) {
    $knownPaths += (Join-Path ${env:ProgramFiles(x86)} 'ChatGPT\ChatGPT.exe')
  }
  $executablePath = $knownPaths |
    Where-Object { $_ -and (Test-Path -LiteralPath $_) } |
    Select-Object -First 1
}

$packages = @(Get-AppxPackage -ErrorAction SilentlyContinue |
  Where-Object { $_.Name -like 'OpenAI.Codex*' -or $_.Name -like 'OpenAI.ChatGPT*' } |
  Sort-Object Version -Descending)

foreach ($package in $packages) {
  if (-not $executablePath) {
    foreach ($relativePath in @('app\ChatGPT.exe', 'ChatGPT.exe')) {
  ...
  if (-not $activationId) {
    try {
      $applicationId = @($package | Get-AppxPackageManifest |
        ForEach-Object { $_.Package.Applications.Application.Id }) |
        Select-Object -First 1
      if ($applicationId) {
        $activationId = "$($package.PackageFamilyName)!$applicationId"
      }
    } catch {}
  }
}

if (-not $executablePath -and -not $activationId) {
  throw '未找到 ChatGPT.exe 或 ChatGPT 的 AppX 注册信息。'
}
...
if ($executablePath) {
  try {
    ...
    if (-not $activationId) { throw }
  }
}

Start-Process -FilePath 'explorer.exe' -ArgumentList "shell:AppsFolder\$activationId" | Out-Null
Write-Output "AppX:$activationId"
"#;
    ...
    if output.status.success() {
        let location = decode_windows_command_output(&output.stdout)
            .trim()
            .to_string();
        return Ok(location);
    }
    ...
    Err(if detail.is_empty() {
        format!("重启 ChatGPT 失败 (exit code: {:?})", output.status.code())
    } else {
        last_lines(detail, 6)
    })
}
