...
const LIBRA_THEME_STORAGE_KEY = "libra-theme";
const LEGACY_THEME_STORAGE_KEY = "cc-switch-theme";

function migrateLegacyThemeStorage(): void {
  try {
    const current = window.localStorage.getItem(LIBRA_THEME_STORAGE_KEY);
    const legacy = window.localStorage.getItem(LEGACY_THEME_STORAGE_KEY);
    if (
      current === null &&
      (legacy === "light" || legacy === "dark" || legacy === "system")
    ) {
      window.localStorage.setItem(LIBRA_THEME_STORAGE_KEY, legacy);
    }
  } catch {
...
async function handleConfigLoadError(
  ...
  const path = payload?.path ?? "~/.libra/config.json";
      ...
      ReactDOM.createRoot(document.getElementById("root")!).render(
        <React.StrictMode>
          <FrontendErrorBoundary>
            <ThemeProvider
              defaultTheme="system"
              storageKey={LIBRA_THEME_STORAGE_KEY}
            >
