...
export function ProviderList({
  ...
  if (isLoading) {
    return (
      <div className="space-y-1 border-y border-border bg-card">
        {[0, 1, 2].map((index) => (
          <div
            key={index}
            className="h-20 w-full animate-pulse border-b border-dashed border-muted-foreground/30 bg-muted/30 last:border-b-0"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="mt-3 space-y-3">
      {claudeDesktopStatusMessages.length > 0 && (
        <div className="border border-amber-500/40 border-l-2 border-l-amber-500 bg-amber-500/10 px-3 py-2.5 text-sm text-amber-900 dark:text-amber-200">
      ...
      <AnimatePresence>
        {isSearchOpen && (
          ...
          >
            <div className="space-y-3 border border-border bg-popover p-3 shadow-xl">
              <div className="relative flex items-center gap-2">
                ...
                <Input
                  ...
                  className="rounded-none border-border bg-background pr-16 pl-9 focus-visible:ring-1 focus-visible:ring-primary"
                />
                {searchTerm && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="absolute right-11 top-1/2 -translate-y-1/2 rounded-none text-xs"
      ...
      {filteredProviders.length === 0 ? (
        <div className="border border-dashed border-border bg-muted/20 px-6 py-8 text-center text-sm text-muted-foreground">
